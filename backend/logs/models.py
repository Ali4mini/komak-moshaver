...
from customer.models import (
    BuyCustomer,
    RentCustomer,
)  # Assuming these are your refactored Customer models
...
class Call(models.Model):
    ...
    phone_number = models.CharField(
        max_length=20, db_index=True, verbose_name="شماره تلفن"
    )
    call_id = models.CharField(
        max_length=100, unique=True, blank=True, null=True, verbose_name="شناسه تماس"
    )
    ...
    duration = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="مدت زمان (ثانیه)"
    )
    ...
    recording_url = models.URLField(
        "آدرس فایل ضبط شده", blank=True, null=True  # Corrected verbose_name
    )
    recording_transcription = models.TextField(
        blank=True, verbose_name="متن پیاده‌سازی شده"
    )
    is_transcript_correct = models.BooleanField(
        default=False, verbose_name="صحت متن پیاده‌سازی شده"
    )
    ...
    location = models.JSONField(
        null=True, blank=True, verbose_name="موقعیت مکانی (اختیاری)"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="زمان ایجاد رکورد"
    )
    updated_at = models.DateTimeField(
        auto_now=True, verbose_name="زمان بروزرسانی رکورد"
    )
    ...
    def save(self, *args, **kwargs):
        if (
            ...
            and self.person
            and hasattr(self.person, "phone_number")
        ):
            self.phone_number = (
                self.person.phone_number
            )  # Make sure Person model has phone_number

        if (
            self.start_time and self.end_time and self.duration is None
...
class BaseActivityLog(models.Model):
    ...
    def __str__(self):
        ...
        file_str = (
            str(self.file.pk) if hasattr(self, "file") and self.file else "فایل نامشخص"
        )
...
class BaseCallLog(BaseActivityLog):
    ...
    subject = models.CharField(
        max_length=2, choices=Subjects.choices, verbose_name="موضوع تماس"
    )
...
class BaseTourLog(BaseActivityLog):
    ...
    tour_type = models.CharField(
        max_length=2, choices=TourTypes.choices, verbose_name="نوع بازدید/مراجعه"
    )
...
class SellCall(BaseCallLog):
    file = models.ForeignKey(
        ...
        related_name="call_logs_sell",
        verbose_name="فایل فروش مرتبط",
    )
    customer = models.ForeignKey(
        BuyCustomer,
        on_delete=models.DO_NOTHING,
        related_name="call_logs_buy",
...
class RentCall(BaseCallLog):
    file = models.ForeignKey(
        Rent,
        on_delete=models.DO_NOTHING,
        related_name="call_logs_rent",
        verbose_name="فایل اجاره مرتبط",
    )
    customer = models.ForeignKey(
        RentCustomer,
        on_delete=models.DO_NOTHING,
        related_name="call_logs_rent_customer",
...
class SellTour(BaseTourLog):
    file = models.ForeignKey(
        Sell,
        on_delete=models.DO_NOTHING,
        related_name="tour_logs_sell",
        verbose_name="فایل فروش مرتبط",
    )
    customer = models.ForeignKey(
        BuyCustomer,
        on_delete=models.DO_NOTHING,
        related_name="tour_logs_buy",
        verbose_name="مشتری خریدار",
    )
...
class RentTour(BaseTourLog):
    file = models.ForeignKey(
        Rent,
        on_delete=models.DO_NOTHING,
        related_name="tour_logs_rent",
        verbose_name="فایل اجاره مرتبط",
    )
    customer = models.ForeignKey(
        RentCustomer,
        on_delete=models.DO_NOTHING,
        related_name="tour_logs_rent_customer",
        verbose_name="مشتری مستاجر",
    )
...
class SMSLog(models.Model):
    task_id = models.CharField(
        max_length=255, unique=True, db_index=True, verbose_name="شناسه تسک"
    )
    status = models.BooleanField(verbose_name="وضعیت ارسال (موفقیت)")
    message = models.TextField(verbose_name="متن پیامک")
    phone_number = models.CharField(
        max_length=20, db_index=True, verbose_name="شماره تلفن گیرنده"
    )  # Max length reduced
    ...
    def resend_sms(self):
        ...
        print(
            f"Attempting to resend SMS task_id: {self.task_id} to {self.phone_number}"
        )
        resend_message.delay(self.phone_number, self.message)


@receiver(post_save, sender=BaseTourLog)
def create_tour_followup_task(sender, instance, created, **kwargs):

    if created:
        print("in created")
        try:
            task = Task.objects.create(
                text=f"پیگری بازدید {instance.customer.id} از {instance.file.id}",
...
post_save.connect(create_tour_followup_task, sender=SellTour)
post_save.connect(create_tour_followup_task, sender=RentTour)
