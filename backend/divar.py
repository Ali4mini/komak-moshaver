...
class Divar:
    ...
    def login(self, phone, cookie=None):
        ...
        if cookie == None:
            ...
            my_divar = self.driver.find_element(
                By.XPATH, "/html/body/div[1]/header/nav/div/div[3]/div/button"
            ).click()
            time.sleep(3)
            login_button = self.driver.find_element(
                By.XPATH, "/html/body/div[1]/header/nav/div/div[3]/div/div/div/button"
            ).click()
            self.driver.implicitly_wait(10)
            time.sleep(2)
            phone_input = self.driver.find_element(
                By.XPATH,
                "//input[@class='kt-textfield__input kt-textfield__input--empty']",
            ).send_keys(phone)
            two_FA_input = input("2FA >>> ")
            two_FA = self.driver.find_element(
                By.XPATH,
                "//input[@class='kt-textfield__input kt-textfield__input--empty']",
            ).send_keys(two_FA_input)
            # time.sleep(4)
    ...
    def first_post(self, page):
        ...
        first = self.driver.find_element(
            By.XPATH, "/html/body/div/div[1]/main/div/div/div/div[1]/a"
        )
    ...
    def post_details(self, post) -> dict:
        ...
        ## login and extract phone number
        phone_button = self.driver.find_element(
            By.CSS_SELECTOR,
            "#app > div.container--has-footer-d86a9.kt-container > div > div > div.kt-col-5 > div.post-actions > button.kt-button.kt-button--primary.post-actions__get-contact",
        ).click()
        time.sleep(10)
        # phone_entery_input = self.driver.find_element(By.XPATH, '//*[@id="textfield-33ljmg5"]').send_keys(self.phone_number)
        # time.sleep(3)
        # # confirm_button = self.driver.find_element(By.XPATH, '/html/body/div[3]/div/div/div/footer/div/button').click()
        # two_FA = self.driver.find_element(By.XPATH, '//*[@id="textfield-20adm6o"]')
        ...
        if self.terms_agreement_done == False:
            try:
                agree_button = self.driver.find_element(
                    By.XPATH, "/html/body/div[2]/div/div/div/footer/button[2]"
                ).click()
        ...
        group_row_title = self.driver.find_elements(
            By.CLASS_NAME, "kt-group-row-item__title"
        )
        group_row_value = self.driver.find_elements(
            By.XPATH, "//span[@class='kt-group-row-item__value']"
        )
        base_row_title = self.driver.find_elements(
            By.XPATH, "//p[@class='kt-base-row__title kt-unexpandable-row__title']"
        )
        ...
        try:
            base_row_value_a_tag = self.driver.find_elements(
                By.XPATH, "//a[@class='kt-unexpandable-row__action kt-text-truncate']"
            )
            base_row_value = self.driver.find_elements(
                By.XPATH, "//p[@class='kt-unexpandable-row__value']"
            )
        ...
        try:
            options = self.driver.find_elements(
                By.XPATH,
                '//*[@id="app"]/div[2]/div/div[1]/div[1]/div[4]/div[7]/div/span',
            )
    ...
    def post_image(self, post: str) -> None:
        self.driver.get(post)
        self.driver.implicitly_wait(10)
        images = self.driver.find_elements(By.XPATH, "//source")
        images = [e.get_attribute("srcset") for e in images]
        return images
