...
class DivarBot:
    ...
    def searcher(self, post, outf="checked_links.txt", inf="checked_links.txt"):
        with open(inf,"r") as last_file:
            last_file = last_file.readline()
            files_to_add = []
            ...
            files_on_divar = self.divar_obj.all_posts(self.divar_url)
            last_file_index = files_on_divar.index(last_file)
            files_to_add = files_on_divar[:last_file_index]
            ...
            with open(outf, 'w') as f1:
                f1.write(post)
                self.logger.info(f"{post}  is last file we extracted ")
