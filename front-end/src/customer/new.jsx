...
const getInitialCustomerType = () => {
  return (localStorage.getItem("agents_field") || "sell").toLowerCase() ===
    "sell"
    ? "buy"
    : "rent";
};
...
const selectBaseClasses =
  "block w-full px-3 py-2.5 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm";
...
const NewCustomer = () => {
  ...
  const resetForm = useCallback(() => {
    ...
    if (upBudgetRef.current) {
      upBudgetRef.current.focus();
    }
  }, []);
  ...
  useEffect(() => {
    if (formData.customerType === "buy") {
      ...
      upBudgetRef.current && upBudgetRef.current.focus();
    }
  ...
  const handleSubmit = useCallback(
    async (event) => {
      ...
      if (
        formData.customerType === "rent" &&
        (!formData.upBudget || !formData.rentBudget)
      ) {
      ...
      if (formData.customerType === "buy") {
        delete customerEntryData.up_budget;
        delete customerEntryData.rent_budget;
      } else if (formData.customerType === "rent") {
        delete customerEntryData.budget;
      }

      if (formData.propertyType === "L") {
        delete customerEntryData.year;
        delete customerEntryData.bedroom;
        delete customerEntryData.vahedha;
        delete customerEntryData.elevator;
      } else if (formData.propertyType === "S") {
        delete customerEntryData.bedroom;
        delete customerEntryData.vahedha;
      }
      ...
      try {
        const response = await api.post(
          `customer/${customerEntryData.customer_type}/new/`,
          customerEntryData,
        );
        if (response.status === 201) {
          dispatch(
            setFlashMessage({
              type: "SUCCESS",
              message: `مشتری با کد ${response.data["id"]} با موفقیت اضافه شد.`,
            }),
          );
          resetForm();
        } else {
          dispatch(
            setFlashMessage({
              type: "WARNING",
              message: `مشتری ثبت شد اما با وضعیت: ${response.status}`,
            }),
          );
          resetForm();
        }
    ...
    <div className="max-w-5xl mx-auto p-6 sm:p-8 bg-white shadow-xl rounded-2xl my-8 ">
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        <fieldset className="p-6 border border-gray-300 rounded-lg">
          <legend className="text-lg font-medium text-indigo-600 px-2">
            اطلاعات پایه مشتری
          </legend>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-8 items-end">
            ...
            <div>
              ...
              <CustomDatePicker
                date={formData.date}
                setter={(d) => handleCustomComponentChange("date", d)}
                inputClassName={selectBaseClasses}
              />
            </div>
          </div>
        </fieldset>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-8">
            {formData.customerType === "buy" ? (
              <FloatLabel
                ...
                ref={budgetRef}
                name="budget"
                label="بودجه کل (تومان)"
                value={formData.budget}
                setter={(val) => handleCustomComponentChange("budget", val)}
                isRequired
              />
            ) : (
              <>
                <FloatLabel
                  ...
                  value={formData.upBudget}
                  setter={(val) => handleCustomComponentChange("upBudget", val)}
                  isRequired
                />
                <FloatLabel
                  type="number"
                  ...
                  setter={(val) =>
                    handleCustomComponentChange("rentBudget", val)
                  }
                  isRequired
                />
              </>
            ...
            <FloatLabel
              type="number"
              name="m2"
              label="متراژ (حداقل متر مربع)"
              value={formData.m2}
              setter={(val) => handleCustomComponentChange("m2", val)}
            />
            ...
            {formData.propertyType !== "L" && (
              <FloatLabel
                ...
                value={formData.year}
                setter={(val) => handleCustomComponentChange("year", val)}
              />
            )}
            {(formData.propertyType === "A" ||
              formData.propertyType === "H") && (
              <FloatLabel
                type="number"
                name="bedroom"
                label="تعداد اتاق خواب (حداقل)"
                value={formData.bedroom}
                setter={(val) => handleCustomComponentChange("bedroom", val)}
              />
            )}
            {formData.propertyType === "A" && (
              <FloatLabel
                type="number"
                name="units"
                label="واحد در طبقه (حداکثر)"
                value={formData.units}
                setter={(val) => handleCustomComponentChange("units", val)}
              />
            )}
          </div>
        </fieldset>
        ...
        <fieldset className="p-6 border border-gray-300 rounded-lg">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
            <FloatLabel
              ...
              setter={(val) =>
                handleCustomComponentChange("customerPhone", val)
              }
              isRequired
              maxChars={11}
              inputMode="numeric"
            ...
            <FloatLabel
              ...
              setter={(val) => handleCustomComponentChange("customerName", val)}
              isRequired
            />
          </div>
        </fieldset>
          ...
          <div className="mt-4 space-y-8">
            ...
            <div>
              <FloatLabel
                type="textarea"
                name="description"
                label="توضیحات و سایر ترجیحات مشتری (اختیاری)"
                value={formData.description}
                setter={(val) =>
                  handleCustomComponentChange("description", val)
                }
              />
            </div>
          </div>
