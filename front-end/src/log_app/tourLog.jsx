...
const NewTourLog = ({ isOpen, setIsOpen }) => {
  ...
  const handleSubmit = (event, entry) => {
    ...
    api
      .post(`logs/${fileType}-tour/`, entry)
      .then(() => {

        dispatch(
          setFlashMessage({
            type: "SUCCESS",
            message: "یک لاگ بازدید اضافه شد",
          }))
        navigate("/")
  ...
  useEffect(() => {
    //TODO: it should request the related customers for this section at first and if the name wasnt find in them we should lookup all the customers
    //WARN:it's very possible that when overtime the number of customers increases this will take time to be done
    api
        ...
        >
          ...
          <div className="fixed inset-0 flex items-center justigy-center">
            <Dialog.Panel className="flex flex-col gap-2 mx-auto w-3/4 px-5 py-2 max-w-md rounded-lg bg-white">
              ...
              <Dialog.Title className=""> لاگ بازدید</Dialog.Title>
              <div className="block border shadow-lg rounded-xl bg-white mx-4 px-4 py-2 my-2">
                ...
                >
                  ...
                  <div className="flex justify-start gap-4">
                    ...
                    <p className="flex justify-center items-center">روش بازدید:</p>
