  ...
  useEffect(() => {
    ...
    const fetchFileDetails = async () => {
      try {
        ...
        const fileDate = fileResponse.data.updated
          ? new Date(fileResponse.data.updated).getTime()
          : null;
        if (fileDate) {
        ...
        const ownerPromise = fileResponse.data.owner
          ? api
              .get(`common/persons/${fileResponse.data.owner}`)
              .then((res) => res)
              .catch((err) => {
                console.warn(
                  `Failed to fetch owner ${fileResponse.data.owner}:`,
                  err.response?.status,
                  err.message,
                );
        ...
        const tenantPromise = fileResponse.data.tenant
          ? api
              .get(`common/persons/${fileResponse.data.tenant}`)
              .then((res) => res)
              .catch((err) => {
                console.warn(
                  `Failed to fetch tenant ${fileResponse.data.tenant}:`,
                  err.response?.status,
                  err.message,
                );
                return { data: null, status: err.response?.status || "error" };
        ...
        const locationPromise = api
          ...
          .catch((err) => {
            ...
            console.error(
              "Error fetching location (non-404):",
              err.response?.status,
              err.message,
            );
            return { data: null, status: err.response?.status || "error" };
        ...
        const [ownerRes, tenantRes, locationRes, imagesRes] = await Promise.all(
          [ownerPromise, tenantPromise, locationPromise, imagesPromise],
        );

        if (ownerRes?.status === 200) setPerson(ownerRes.data);
        if (tenantRes?.status === 200) setTenant(tenantRes.data);
        if (locationRes?.status === 200) setLocation(locationRes.data);
        else setLocation(null);
        if (imagesRes?.status === 200) setImages(imagesRes.data);
        else setImages([]);
      } catch (err) {
        console.error(
          "Critical error fetching file details:",
          err.response?.status,
          err.message,
        );
        if (err.response && err.response.status === 404) {
          setError(
            `فایل با شناسه ${id} یافت نشد. ممکن است حذف شده باشد یا شناسه نامعتبر باشد.`,
          );
        } else {
          setError(
            "مشکلی در بارگذاری اطلاعات اصلی فایل پیش آمده است. لطفا صفحه را رفرش کنید یا بعدا تلاش نمایید.",
          );
        }
        setFile(null);
      } finally {
  ...
  const optionItems = [
    {
      key: "callLog",
      label: "لاگ تماس",
      handler: () => setIsCallLog(true),
      icon: <PhoneIcon />,
    },
    {
      key: "addressSMS",
      label: "ارسال آدرس (SMS)",
      handler: () => setIsAddressSMS(true),
      icon: <ChatBubbleLeftEllipsisIcon />,
    },
    {
      key: "tourLog",
      label: "لاگ بازدید",
      handler: () => setIsTourLog(true),
      icon: <HomeIcon />,
    },
    {
      key: "updated",
    ...
    },
    {
      ...
      label: "مشتریان مرتبط",
      handler: () => setIsMatchedCustomer(true),
      icon: <UsersIcon />,
    },
    {
      key: "delete",
    ...
    <div className="bg-white shadow-xl rounded-xl p-5 md:p-6 mb-6">
      {title && (
        <div className="flex items-center mb-4 border-b border-gray-200 pb-3">
          {icon && <div className="me-3 text-blue-600">{icon}</div>}{" "}
          {/* me-3 for RTL */}
          <h2
            className={`text-xl font-semibold text-gray-700 ${titleClassName}`}
          >
            {title}
          </h2>
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen p-6 text-center bg-gray-100">
        ...
        <p className="text-gray-600 mb-8">
          اطلاعات این فایل در دسترس نیست یا ممکن است شناسه نامعتبر باشد.
        </p>
        <button
          onClick={() => navigate("/")}
          className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-150"
    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-6 lg:p-8">
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-x-4 gap-y-3">
        <div className="flex-grow min-w-0">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 truncate">
            جزئیات فایل{" "}
            <span className="text-gray-500 font-normal">(کد: {id})</span>
          </h1>
          ...
          <div className="flex items-center gap-1 bg-white shadow-sm rounded-lg border border-slate-200 p-0.5 w-full sm:w-auto">
            <SimpleTooltip text="ویرایش فایل" position="top">
              ...
              >
                ...
                <span className="hidden md:inline ms-1.5 text-sm font-medium">
                  ویرایش
                </span>{" "}
                {/* ms-1.5 for RTL */}
              </button>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-3.5">
              <InfoItem
                label="نوع فایل"
                value={fileType === "sell" ? "فروش" : "اجاره"}
                icon={<TagIcon className="w-5 h-5" />}
              />
              <InfoItem
                label="نوع ملک"
                value={file.property_type_display || file.property_type}
              ...
              <InfoItem
                ...
                value={file.persian_updated || "نامشخص"}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-3.5">
              ...
              {fileType === "sell" ? (
                <InfoItem
                  label="قیمت کل (تومان)"
                  value={
                    file.price ? Number(file.price).toLocaleString() : null
                  }
                ...
                <>
                  <InfoItem
                    ...
                    value={
                      file.price_up
                        ? Number(file.price_up).toLocaleString()
                        : null
                    }
                    icon={<CurrencyDollarIcon className="w-5 h-5" />}
                  />
                  <InfoItem
                    ...
                    value={
                      file.price_rent
                        ? Number(file.price_rent).toLocaleString()
                        : null
                    }
                    icon={<CurrencyDollarIcon className="w-5 h-5" />}
                  />
              ...
              <InfoItem
                label="تبدیل"
                value={
                  file.tabdil ? "دارد" : file.tabdil === false ? "ندارد" : null
                }
              />
              <InfoItem
                label="وضعیت بازدید"
                value={file.bazdid_display || file.bazdid || "هماهنگ شود"}
                ...
                {tenant.phone_number && (
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <InfoItem
                      ...
                      valueClassName="cursor-pointer text-blue-600 hover:underline"
                      onClick={() =>
                        person.id && navigate(`/persons/${person.id}`)
                      }
                    />
                    <InfoItem
                      label="شماره مستاجر"
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap p-3 bg-gray-50 rounded-md border border-gray-200">
                  {file.address || "آدرس ثبت نشده است."}
                </p>
              </div>
              <div>
                <h3 className="font-medium text-gray-700 mb-1">توضیحات:</h3>
                <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap p-3 bg-gray-50 rounded-md border border-gray-200">
                  {file.description || "توضیحاتی ثبت نشده است."}
                </p>
              </div>
            </div>
          </SectionCard>
                ...
                {features.map(
                  ({ key, icon, text }) =>
                    file?.[key] && (
                      <div
                        key={key}
                        className="flex flex-col items-center justify-center p-3 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-all"
                      >
              ...
              <div className="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden shadow-md border border-gray-200">
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (
                      fallbackDiv &&
                      fallbackDiv.classList.contains("map-fallback")
                    ) {
      ...
      {isDeleteConfirm && file && (
        <DeleteConfirm
          ...
          description={
            "این عملیات غیرقابل بازگشت است. اگر فایل را اشتباهی پاک کردید سریعا به مدیر مجموعه اطلاع رسانی کنید."
          }
          onConfirm={() => {
            api
              .delete(`file/${fileType}/${id}/`)
              .then(() => {
                setIsDeleteConfirm(false);
                navigate("/", {
                  replace: true,
                  state: { message: "فایل با موفقیت حذف شد." },
                });
              })
              .catch((err) => {
                console.error(
