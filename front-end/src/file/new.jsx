...
const NewFile = () => {
  const initialFileType = (
    localStorage.getItem("agents_field") || "sell"
  ).toLowerCase();
  ...
  const create = async (event) => {
    ...
    if (tenantPhone && tenantPhone.length !== 11) {
    ...
    try {
      const response = await api.post(
        `file/${fileEnteryData.file_type}/new/`,
        fileEnteryData,
      );
  ...
  const selectClasses =
    "block w-full px-3 py-2.5 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-center";
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value === "no") setTabdil("");
                    }}
                    className={selectClasses}
                  >
