...
const NewFileIcon = ({ className = "w-5 h-5" }) => (
  <svg className={className} viewBox="0 0 20 20" fill="currentColor">
    ...
    <path
      fillRule="evenodd"
      d="M10 12a1 1 0 011 1v1h1a1 1 0 110 2h-1v1a1 1 0 11-2 0v-1H8a1 1 0 110-2h1v-1a1 1 0 011-1z"
      clipRule="evenodd"
    />
  </svg>
);
...
const NavBar = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((error) =>
        console.log("Error fetching restore files count:", error),
      );
  }, []);

  const navItemTextClasses = `ml-3 text-sm font-medium whitespace-nowrap transition-opacity duration-150 ease-in-out 
  ...
  const NavLinkItem = ({ to, icon: Icon, label, badgeCount }) => {
    const commonItemClasses =
      "flex items-center h-11 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-60 transition-colors duration-150 group relative";
    ...
    return (
      <div className="relative">
        ...
        {!isExpanded && (
          <span
            className="absolute top-1/2 -translate-y-1/2 right-full mr-3 z-20
  ...
  const NavButtonItem = ({ onClick, icon: Icon, label }) => {
    const commonItemClasses =
      "flex items-center h-11 w-full rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-60 transition-colors duration-150 group relative";
    const expandedClasses =
      "px-3 justify-start text-gray-700 hover:bg-gray-100";
    const collapsedClasses =
      "w-11 justify-center text-gray-600 hover:bg-gray-200";
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`flex items-center mb-4 h-10 ${isExpanded ? "justify-between" : "justify-center"}`}
        >
          {isExpanded && (
            <Link
              to="/dashboard"
              className="text-xl font-semibold text-blue-700 ml-2"
            >
              CRM من
            </Link>
          )}
